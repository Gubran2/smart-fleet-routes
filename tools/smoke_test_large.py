"""Smoke test for large problem sizes: 1000 points and 2 vehicles.
Runs the same pipeline as tools/smoke_test.py but with larger inputs to measure performance
in the 'many points, few vehicles' regime.
"""
import time
import math
import random
...
def generate_points_on_roads(edges, cum, tot, n_points, min_spacing_m, seed=42):
    ...
    pts = []
    tries, max_tries = 0, max(20000, n_points * 40)
...
if __name__ == '__main__':
    print('Starting large smoke test...')
    t0 = time.perf_counter()
    ...
    t3 = time.perf_counter()
    results = []
    # We still use parallel execution but only 2 clusters -> parallelism limited
